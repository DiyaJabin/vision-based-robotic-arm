...
def _normalize_trial_record(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    latency = record.get("perception_latency_ms")
    row["perception_latency_ms"] = None if latency is None else _coerce_numeric(latency)
    row["ik_feasible"] = _coerce_bool(record.get("ik_feasible", False))
    row["grasp_attempted"] = _coerce_bool(record.get("grasp_attempted", False))
    row["grasp_success"] = _coerce_bool(record.get("grasp_success", False))
    row["placement_success"] = _coerce_bool(record.get("placement_success", False))
...
def aggregate_trials(rows: Iterable[Mapping[str, Any]] | pd.DataFrame) -> dict[str, Any]:
    ...
    success_count = int(frame["placement_success"].astype(float).sum())
    ...
    return {
        ...
        "mean_latency_ms": float(latency_source.mean()) if total_trials else 0.0,
        "median_latency_ms": float(latency_source.median()) if total_trials else 0.0,
